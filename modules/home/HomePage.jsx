...
import { useContract, useFx_FxETHTwapOracle } from '@/hooks/useContracts'
...
export default function HomePage() {
  ...
  const handleSetStETHPrice = async () => {
    try {
      setPriceLoading(true)
      const _minOut = cBN(stEthPrice).toString(10)
  ...
  const handlePrice = async () => {
    ...
    console.log('aaa----', _priceInfo, _ema_priceInfo)
  }
  ...
  return (
    <div className={styles.container}>
      <div className={styles.item}>
        ...
        <div>
          isValid : {pricePriceInfo._isValid ? 'true' : 'false'}
          <br />
          maxUnsafePrice:{' '}
          {checkNotZoroNumOption(
            pricePriceInfo._maxUnsafePrice,
            pricePriceInfo._maxUnsafePrice / 1e18
          )}
          <br />
          minUnsafePrice:
          {checkNotZoroNumOption(
            pricePriceInfo._minUnsafePrice,
            pricePriceInfo._minUnsafePrice / 1e18
          )}
          <br />
          safePrice:
          {checkNotZoroNumOption(
            pricePriceInfo._safePrice,
            pricePriceInfo._safePrice / 1e18
          )}
          <br />
          emaPrice:{checkNotZoroNumOption(emaPriceInfo, emaPriceInfo / 1e18)}
        </div>
